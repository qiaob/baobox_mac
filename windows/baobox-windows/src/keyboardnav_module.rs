...
//! # 覆盖层是分层窗口 + color key
//!
//! 与截图覆盖层同一个套路：全屏铺一个 `WS_EX_LAYERED` 窗口，
//! 底色刷成约定的「透明色」再整片透过去，标签画在这块布上。
//!
//! # 键盘滚动没有做
//!
...
//! # 扫描要在覆盖层出现**之前**
//!
//! 反过来的话，覆盖层自己会成为前台窗口 —— UI Automation 扫的就是它，
//! 而它里面什么可点的东西都没有。
...
impl KeyboardNavTool {
    ...
    fn run_click(&mut self) -> Result<String, String> {
        ...
        for hit in &picked {
            keyboardnav::click(hit.target.x, hit.target.y)?;
        }
