//! 防休眠工具接进 App 框架（Linux）。
//!
//! 时长预设、到期判定、状态文案都在 `baobox_core::caffeinate`，
//! 让系统别睡在 `caffeinate`。这个文件只回答框架的问题。
//!
//! # 到期靠 `tick`，不另起线程
//!
...
//! 让托盘菜单的状态行跟上。
//!
//! 代价是精度只有一个 tick 周期（50ms），而这是个以分钟计的功能。
...
pub struct CaffeinateTool {
    ...
    /// 握着它期间系统不睡；丢掉就是解除
    inhibitor: Option<caffeinate::Inhibitor>,
}
...
impl CaffeinateTool {
    ...
    fn start(&mut self, duration: Option<i64>) -> Result<String, String> {
        // 先把旧的解除掉 —— 两个抑制叠着不会更管用，只会让解除漏掉一个
        self.stop();
        let inhibitor = caffeinate::engage(self.settings.prevent_display_sleep)?;
        let note = inhibitor.describe();
