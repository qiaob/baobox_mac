...
enum ScreenshotResultHandler {
    ...
    private static func saveToDisk(png: Data) {
        ...
        if base.isEmpty {
            formatter.dateFormat = ScreenshotSettings.defaultTemplate
            base = formatter.string(from: Date())
        }

        do {
            try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
            try png.write(to: uniqueURL(in: dir, base: base))
        } catch {
    ...
    private static func uniqueURL(in dir: URL, base: String) -> URL {
        let first = dir.appendingPathComponent(base + ".png")
        guard FileManager.default.fileExists(atPath: first.path) else { return first }
        for n in 2...999 {
            let candidate = dir.appendingPathComponent("\(base)-\(n).png")
            if !FileManager.default.fileExists(atPath: candidate.path) { return candidate }
        }
        return dir.appendingPathComponent("\(base)-\(UUID().uuidString).png")
    }
}
