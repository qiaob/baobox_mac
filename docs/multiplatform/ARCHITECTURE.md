...
`mac/` 里是完整可用的 0.0.6；`windows/` 与 `linux/` 是新起的实现，当前覆盖**截图**、**剪贴板**、**防休眠**、**窗口管理**、**Claude Code 助手**、**Codex 助手**、**键盘点击**七个工具。
...
第七个工具。标签怎么发、打字之后怎么反应在 `baobox_core::hints`；
平台层只回答两个问题：**屏幕上有哪些可点的东西**、**怎么点**。
...
- **标签必须互不为前缀**，这是整个功能的核心约束。如果 `a` 和 `ab` 同时是标签，
  用户打完 `a` 我们就得**等** —— 等着的时候屏幕上什么都没发生，用户只会以为
  程序卡了然后再按一次。用 Vimium 那套 BFS 发号（前缀被展开时自己就不再是标签），
  发出来的一定互不为前缀，打完最后一个字符立刻动手。
- **不用无障碍接口自带的「按一下」**。AT-SPI 有 `Action`、UIA 有 `Invoke`，
  但很多程序（Electron、游戏、老 MFC、自绘界面）只实现了「报告」没实现「执行」，
  调了毫无反应**而且不报错**。合成一次真实的鼠标点击对谁都成立。
- **Linux 上无障碍可能是关着的**，那时一个元素都扫不到。这时必须明说要开什么、
  怎么开，而不是显示「这个窗口里没有可点的东西」—— 后者会让用户以为是自己
  找错了地方，然后反复按快捷键。
...
（`hints::Scroll`）已经写好并测过，缺的只是喂按键那一层。

### 其余一个工具

| 工具 | 难度 | 关键点 | 为什么还没做 |
|---|---|---|---|
| 网络抓包 | 🔴 | HTTPS MITM 代理 + 自签 CA | 需要一整套 TLS 栈与证书签发。mac 那边是 Network.framework + 外挂 `/usr/bin/openssl`；Rust 侧要引 rustls + rcgen，是一块独立的、安全敏感的工程 |
