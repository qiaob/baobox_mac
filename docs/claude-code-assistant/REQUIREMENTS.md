...
| 16 | 最近文件 | 快速续接面板新增「文件」模式(⇥ 切换):列出各会话通过 Write/Edit 写过的文件(全量历史,按路径去重取最近),类型筛选 chips(文档/网页/代码/配置/其他),⏎ 按类别偏好应用打开、⌘⏎ 访达显示、⌘C 复制路径;菜单「最近文件…」直达,快捷键出厂不绑定;设计见 `RECENT_FILES.md` | 面板文件模式列出会话写过的文件;筛「文档」只剩 md/txt 等;⏎ 用所配应用打开,未配走系统默认 |
